...
* When containing child nodes, it is referred to as a [container tag.](#container-tags)
* It is stored in the repository below a base path called the [taxonomy root node.](#taxonomy-root-node)

### TagID {#tagid}

A `TagID` identifies a path which resolves to a tag node in the repository.

Typically, the `TagID` is a shorthand starting with the namespace or it can be absolute starting from the [taxonomy root node](#taxonomy-root-node).
...
|`TagID`|Namespace|Local ID|Container Tag(s)|Leaf Tag|Absolute Repository Path|
|---|---|---|---|---|---|
...
Tags exist as nodes in the repository under the [taxonomy root node.](#taxonomy-root-node) Allowing or denying authors and site visitors the ability to create tags in a given namespace can be achieved by setting appropriate ACLs in the repository.
