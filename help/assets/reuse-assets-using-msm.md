...
* Create assets once and then make copies of these assets for re-use in other areas the same or other sites.
...
Live copy
: The copy of the source assets/folders that is in synchronization with its source.
...
Rollout
: An action that pushes the modifications made to the source downstream to its live copies. It is possible to update one or more live copies in one go using rollout action.

Rollout config
: Rules that determine which properties will be synchronized, how and when. These configurations are applied when creating live copies; can be edited later; and a child can inherit rollout configuration from its parent asset. For MSM for Assets, use only the Standard rollout config. The other rollout configurations are not available for MSM for Assets.

Synchronize
: Another action, in addition to rollout, that brings parity between source and its live copy by sending the updates from source to live copies. A sync is initiated for a particular live copy and the action pulls the changes from the source. Using this action, is possible to only update one of the live copies.

Suspend
: Temporarily remove the live relationship between a live copy and its source asset/folder. This can be resumed.

Resume
: Resume the live relationship so that a live copy again starts receiving the updates from source.

Detach
: Irrevocably remove the live relationship of a live copy asset/folder. After detach action, the live copies can never receive updates from source and it stops being a live copy anymore.

Reset
: Reset action makes the live copy again a replica of the source by overwriting any local changes. It also removes inheritance cancellations and resets inheritance on all metadata fields. To make local modifications in the future, you must again cancel the inheritance of specific fields.
...
* **Method 1**: Select the source asset(s) and click **[!UICONTROL Create > Live Copy]** from toolbar at the top.
* **Method 2**: In AEM user interface, click **[!UICONTROL Create > Live Copy]** from top right corner of the interface.
...
Content Fragments (CFs) are not supported for the use case. When attempting to create their live copies, CFs are copied over as is without any relationship. The copied CFs is a snapshot in time and does not update when original CFs is updated.
...
1. In AEM interface, from top right corner, click **[!UICONTROL Create > Live Copy]**.
...
>[!TIP]
>
>To check the status of a few separate live copies, use the first method that is see the **[!UICONTROL Properties]** page. To check statuses of a large number of live copies, use the second method, that is, see **[!UICONTROL Relationship Status]** page.
...
1. Click **[!UICONTROL Live Copy Source]**. Click **[!UICONTROL Live Copy Overview]** to open the console. This dashboard provides a top-level status of all the child assets.
...
After a source is modified the changes can be propogated to the live copies using either a synchronize action or a rollout action. See [glossary](#glossary-msm-for-assets) to understand the difference between both the actions.
...
A live copy is a replica of the original source when it is created. The metadata values of a live copy are inherited from the source. The metadata fields individually maintain an inheritance with the respective fields of the source asset.

However, you have the flexibility to make local modifications to a live copy to change a few select properties. To make local modifications, cancel the inheritance of the desired property. When inheritance of one or more metadata fields is canceled, the live relationship of the asset as well as the inheritance of the other metadata fields is retained. Any synchronization or rollout does not overwrite the local modifications. To do so, open **[!UICONTROL Properties]** page of a live copy asset, click on **[!UICONTROL cancel inheritance]** icon next to a metadata field.
...
You can completely remove any and all relationship between a source and a live copy using Detach action. The live copy becomes a stand-alone asset or folder after it is detached. It is displayed as a new asset in AEM interface, immediately after detaching. To detach a live copy from its source, follow these steps.
...
1. In this dashboard, select a live copy asset from a live copy folder. Click on the desired actions from the toolbar. The actions available are **[!UICONTROL Edit]**, **[!UICONTROL Synchronize]**, **[!UICONTROL Reset]**, **[!UICONTROL Suspend]**, and **[!UICONTROL Detach]**. You can quickly initiate these actions on any asset in any number of live copy folders that are in a live relationship with the selected source folder.
