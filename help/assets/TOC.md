---
cloud: experience-cloud
product: Adobe Experience Manager
audience: end-user
user-guide-title: AEM 6.4 Assets User Guide
---

# AEM 6.4 Assets User Guide {#assets}

+ Introduction to AEM Assets {#introduction}
    + [Managing Assets with the Touch-Optimized UI](managing-assets-touch-ui.md)
    + [Working with Content Fragments](content-fragments.md)
    + [Working with Dynamic Media](dynamic-media.md)
    + [Working with 3D assets](3d-assets.md)
    + [Naming conventions for assets](asset-naming-conventions.md)
    + [User Experience Improvements](ux-improvements.md)
+ Authoring AEM Assets {#authoring}
    + [GQL Full-text Search](gql-search.md)
    + [Asset Reports](asset-reports.md)
    + [Search assets in AEM](search-assets.md)
    + [Review folder assets and collections](bulk-approval.md)
    + [Activity stream in timeline](activity-stream.md)
    + [Multilingual Assets](multilingual-assets.md)
+ Dynamic Media{#dynamic} 
    + [Setting Up Dynamic Media](administering-dynamic-media.md)
    + [Configuring Dynamic Media - Scene7 mode](config-dms7.md)
    + [Configuring Dynamic Media - Hybrid mode](config-dynamic.md)
    + [Troubleshooting Dynamic Media - Scene7 mode](troubleshoot-dms7.md)
    + [Managing Assets](managing-assets.md)
    + [Best practices for optimizing the quality of your images](best-practices-for-optimizing-the-quality-of-your-images.md)
    + [Managing Viewer Presets](managing-viewer-presets.md)
    + [Applying Viewer Presets](viewer-presets.md)
    + [Managing Image Presets](managing-image-presets.md)
    + [Applying Image Presets](image-presets.md)
    + [Smart Imaging](imaging-faq.md)
    + [HTTP2 Delivery of Content](http2.md)
    + [Delivering Dynamic Media Assets](delivering-dynamic-media-assets.md)
    + [Activating hotlink protection in Dynamic Media](hotlink-protection.md)
    + [Image Sets](image-sets.md)
    + [Mixed Media Sets](mixed-media-sets.md)
    + [Spin Sets](spin-sets.md)
    + [Panoramic Images](panoramic-images.md)
    + [Video](video.md)
    + [Interactive Images](interactive-images.md)
    + [Interactive Videos](interactive-videos.md)
    + [Carousel Banners](carousel-banners.md)
    + [Using Quickviews to create custom pop-ups](custom-pop-ups.md)
    + [Delivering Optimized Images for a Responsive Site](responsive-site.md)
    + [Previewing Assets](previewing-assets.md)
    + [Adding Dynamic Media Assets to Pages](adding-dynamic-media-assets-to-pages.md)
    + [Embedding the Video or Image Viewer on a Web Page](embed-code.md)
    + [Linking URLs to your Web Application](linking-urls-to-yourwebapplication.md)
    + [Using Rulesets to Transform URLs](using-rulesets-to-transform-urls.md)
    + [Publishing Dynamic Media Assets](publishing-dynamicmedia-assets.md)
    + [Invalidating your CDN cached content](invalidate-cdn-cached-content.md)
    + [Installing Feature Pack 18912](bulk-ingest-migrate.md)
+ Extending Assets {#extending}  
    + [Customizing and Extending Assets](extending-assets.md)
    + [Processing Assets Using Media Handlers and Workflows](media-handlers.md)
    + [Extending Assets Search](searchx.md)
    + [Install and configure ImageMagick to work with AEM Assets](best-practices-for-imagemagick.md)
    + [Integrating Assets with Activity Stream](extending-activity-stream.md)
    + [Extending Asset Editor](asseteditorx.md)
+ Administering Assets {#administer}
    + [Search Facets](search-facets.md)
    + [Setting Up Dynamic Media](administering-dynamic-media.md)
    + [XMP writeback to renditions](xmp-writeback.md)
    + [Asset link sharing](link-sharing.md)
    + [Best Practices for Assets](best-practices-for-assets.md)
    + [Managing Metadata for Digital Assets](metadata.md)
+ Brand Portal {#brandportal}
    + [Configure AEM Assets integration with Brand Portal](brand-portal-configuring-integration.md)
    + [Publish collections to Brand Portal](brand-portal-publish-collection.md)
    + [Publish folders to Brand Portal](brand-portal-publish-folder.md)
+ Content Fragments {#fragments}
    + [Working with Content Fragments](content-fragments.md)
    + [Managing Content Fragments](content-fragments-managing.md)
    + [Content Fragment Models](content-fragments-models.md)
    + [Variations - Authoring Fragment Content](content-fragments-variations.md)
    + [Content Fragment Associated Content](content-fragments-assoc-content.md)
    + [Metadata - Fragment Properties](content-fragments-metadata.md)
    + [Content Fragments - Delete Considerations](content-fragments-delete.md)
    + [Content Fragments - Markdown](content-fragments-markdown.md)
+ Managing Assets {#managing}
    + [Asset Selector](asset-selector.md)
    + [Creating Translation Projects for Content Fragments](creating-translation-projects-for-content-fragments.md)
    + [Check In and Check Out Files in Assets](check-out-and-submit-assets.md)
    + [Configure Asset tagging using the Smart Content Service](config-smart-tagging.md)
    + [Enabling Duplicate Detection](duplicate-detection.md)
    + [Enhanced sorting of assets in AEM](enhance-sort.md)
    + [Video](s7-video.md)
    + [Adding Scene7 Features to your Page](scene7.md)
+ 3D Assets {#3d}
    + [AEM 3D Release Note](/help/release-notes/aem3d-release-notes.md)
    + [Installing and configuring AEM 3D](install-config-3d.md)
    + [Advanced configuration settings](advanced-config-3d.md)
    + [Working with Adobe Dimension assets](working-dimension-assets.md)
    + [About the uploading and processing of 3D assets in AEM](upload-processing-3d-assets.md)
    + [Resolving file dependencies](resolve-file-dependencies.md)
    + [Viewing 3D assets](viewing-3d-assets.md)
    + [Rendering 3D assets](rendering-3d-assets.md)
    + [About the use of stages in AEM 3D](about-the-use-of-stages-in-aem-3d.md)
    + [Setting up a standard stage with Autodesk Maya and Mental Ray](setting-up-stage-with-ad-mr.md)
    + [Working with the 3D Sites component](using-the-3d-sites-component.md)
    + [Integrating AEM 3D with Autodesk Maya](integrate-maya-with-3d.md)
    + [About working with IBL stages](working-with-ibl-stages.md)
    + [Integrating AEM 3D with Autodesk 3ds Max](integrating-aem-3d-with-autodesk-3ds-max.md)
    + [Setting up an IBL stage with Autodesk Maya and Mental Ray](setting-up-an-ibl-stage-with-autodesk-maya-and-mental-ray.md)
+ Deleted section Remove duplicates and move others to sections above {#deleted}
    + [Getting Started with AEM Assets](getting-started-with-aem-assets.md)
    + [AEM Desktop App Release Notes](desktop-app-release-notes.md)
    + [Assets HTTP API](mac-api-assets.md)
    + [Extending Assets Search](searchx.md)
    + [Managing Assets with the Touch-Optimized UI](managing-assets-touch-ui.md)
    + [Assets Offloading Best Practices](assets-offloading-best-practices.md)
    + [Creating Translation Projects](translation-projects.md)
    + [Customizing Asset Share](assetsharex.md)
    + [Configure Asset tagging using the Smart Content Service](config-smart-tagging.md)
    + [Extending Asset Editor](asseteditorx.md)
    + [Variations - Authoring Fragment Content](content-fragments-variations.md)
    + [Managing Collections](managing-collections-touch-ui.md)
    + [Integrating Assets with Activity Stream](extending-activity-stream.md)
    + [Preparing Assets for Translation](preparing-assets-for-translation.md)
    + [Assets file format best practices](assets-file-format-best-practices.md)
    + [Install and configure ImageMagick to work with AEM Assets](best-practices-for-imagemagick.md)
    + [Asynchronous Operations](asynchronous-jobs.md)
    + [Applying Workflows to Assets](assets-workflow.md)
    + [Working with Content Fragments](content-fragments.md)
    + [Assets Monitoring Best Practices](assets-monitoring-best-practices.md)
    + [Support for IPTC Metadata](iptc-support.md)
    + [Digital Rights Management in Assets](drm.md)
    + [How to edit or add metadata](meta-edit.md)
    + [Metadata Schemas](metadata-schemas.md)
    + [Lightbox](touch-ui-light-box.md)
    + [Enabling Asset Insights through DTM](touch-ui-using-dtm-for-asset-insights.md)
    + [Imaging Transcoding Library](imaging-transcoding-library.md)
    + [Cascading Metadata](cascading-metadata.md)
    + [Content Fragment Models](content-fragments-models.md)
    + [Associated Content](content-fragments-assoc-content.md)
    + [Asset Reports](asset-reports.md)
    + [Configuring Asset Insights](touch-ui-configuring-asset-insights.md)
    + [Applying translation cloud services to folders ](transition-cloud-services.md)
    + [Managing Smart Tags](managing-smart-tags.md)
    + [Assets Migration Guide](assets-migration-guide.md)
    + [GQL Full-text Search](gql-search.md)
    + [Assets Proxy Development](proxy.md)
    + [Customizing and Extending Assets](extending-assets.md)
    + [AEM Assets Home Page Experience](assets-home-page.md)
    + [Using Page Tracker and Embed code in web pages](touch-ui-using-page-tracker.md)
    + [Publish folders to Brand Portal](brand-portal-publish-folder.md)
    + [Configure AEM Assets integration with Brand Portal](brand-portal-configuring-integration.md)
    + [Serial Uploads](serial-uploads.md)
    + [Managing Compound Assets](managing-linked-subassets.md)
    + [DO NOT PUBLISH | New features summary | AEM Assets](whats-new.md)
    + [Related Assets](related-assets.md)
    + [Integrating AEM Assets with InDesign Server](indesign.md)
    + [Watermarking](watermarking.md)
    + [Connecting Assets to Desktop Applications with Adobe Drive](desktop-apps-with-drive.md)
    + [AEM and Creative Cloud Integration Best Practices](aem-cc-integration-best-practices.md)
    + [Administering Assets](assets.md)
    + [Asset Insights](touch-ui-asset-insights.md)
    + [Asset Templates](asset-templates.md)
    + [Detecting MIME Type of Assets Using Apache Tika](detect-asset-mime-type-with-tika.md)
    + [Camera Raw Support](camera-raw.md)
    + [XMP Metadata](xmp.md)
    + [XMP writeback to renditions](xmp-writeback.md)
    + [Assets Network Considerations](assets-network-considerations.md)
    + [AEM Assets vs. AEM MediaLibrary](medialibrary.md)
    + [Managing Metadata for Digital Assets](metadata.md)
    + [Publish collections to Brand Portal](brand-portal-publish-collection.md)
    + [Content Fragments - Delete Considerations](content-fragments-delete.md)
    + [Bulk Metadata Import and Export](metadata-import-export.md)
    + [DO NOT PUBLISH | Enable Amazon Cloudfront to serve content](serving-content-directly-using-amazon-cloudfront.md)
    + [Best Practices for Assets](best-practices-for-assets.md)
    + [DO NOT PUBLISH | User Experience Improvements](ux-improvements.md)
    + [Multilingual Assets](multilingual-assets.md)
    + [Search assets in AEM](search-assets.md)
    + [Naming conventions for assets](asset-naming-conventions.md)
    + [Enabling Duplicate Detection](duplicate-detection.md)
    + [Using PDF Rasterizer](aem-pdf-rasterizer.md)
    + [Configuring Asset Upload Restrictions](configuring-asset-upload-restrictions.md)
    + [Use Adobe Stock assets in AEM Assets](aem-assets-adobe-stock.md)
    + [Installing and Configuring Video Transcoder with FFmpeg](video-transcoder.md)
    + [Creating and Configuring Asset Editor pages](assets-finder-editor.md)
    + [Enhanced sorting of assets in AEM](enhance-sort.md)
    + [Searching Video Assets](search-video-assets.md)
    + [Managing Multiple Assets and Collections](managing-multiple-assets.md)
    + [[Redirect] Authoring Adobe Flash Content](flash.md)
    + [Assets Sizing Guide](assets-sizing-guide.md)
    + [Review folder assets and collections](bulk-approval.md)
    + [Multi-tenancy for Collections, Snippets, and Snippet Templates](multi-tenancy.md)
    + [AEM to Creative Cloud Folder Sharing Best Practices](aem-cc-folder-sharing-best-practices.md)
    + [Private folder sharing](private-folder.md)
    + [Search Facets](search-facets.md)
    + [Markdown](content-fragments-markdown.md)
    + [Folder Metadata Schema](folder-metadata-schema.md)
    + [Managing Content Fragments](content-fragments-managing.md)
    + [Adding Image Maps](image-maps.md)
    + [Using demo package for Asset Insights](touch-ui-using-demo-package-for-asset-insights.md)
    + [Enhanced Smart Tags](enhanced-smart-tags.md)
    + [Processing Assets Using Media Handlers and Workflows](media-handlers.md)
    + [Metadata Schemata Reference](meta-ref.md)
    + [Asset Selector](asset-selector.md)
    + [Metadata - Fragment Properties](content-fragments-metadata.md)
    + [Smart Content Service Training Guidelines](smart-tags-training-guidelines.md)
    + [Creating Translation Projects for Content Fragments](creating-translation-projects-for-content-fragments.md)
    + [Best Practices for Translating Assets Efficiently](best-practices-for-translating-assets-efficiently.md)
    + [Activity stream in timeline](activity-stream.md)
    + [Check In and Check Out Files in Assets](check-out-and-submit-assets.md)
    + [Video renditions](video-renditions.md)
    + [Metadata Profiles](metadata-profiles.md)
    + [Assets Performance Tuning Guide](performance-tuning-guidelines.md)
    + [Asset link sharing](link-sharing.md)
    + [Working with Selectors](working-with-selectors.md)
    + [RICK BROUGH -ADMINISTERING Video Profiles](video-profiles.md)
    + [RICK BROUGH -ADMINISTERING Image Profiles](image-profiles.md)
    + [RICK BROUGH -ADMINISTERING Best Practices for Organizing Your Digital Assets for Using Profiles](best-practices-for-file-management.md)
    + [RICK BROUGH -ADMINISTERING Profiles for Processing Metadata, Images, and Videos](processing-profiles.md)
    + [RICK BROUGH -ADMINISTERING Assets Supported Formats](assets-formats.md)
    + [RICK BROUGH -MANAGING ASSETS Managing Video Assets](managing-video-assets.md)
    + [RICK BROUGH -MANAGING ASSETS Organize Digital Assets](organize-assets.md)
    + [RICK BROUGH -DYNAMIC MEDIA TOPIC IS EMPTY Images](dynamic-media-images.md)
    + [RICK BROUGH -NEW TOPIC BUT NOT FINISHED DO NOT PUBLISH Integrating digital non-visible watermarking (Digimarc) with Dynamic Media](digimarc-integration.md)