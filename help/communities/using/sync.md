...
On author, when a community site is published from the [Communities Sites console](/help/communities/using/sites-console.md), the effect is to [replicate](/help/sites/deploying/using/configuring.md#replication-reverse-replication-and-replication-agents) the associated pages, and Sling distribute the dynamically created community user groups, including their membership.
...
1. **Check configurations**. Ensure that all the [configurations](/help/communities/using/sync.md#bestpractices)are appropriately set in your AEM Author instance, as mentioned in the [Best Practices](/help/communities/using/sync.md#main-pars-header-863110628).
...
1. **Check logs generated on user creation on Publish instance**. If the above configurations are appropriately set yet user sync is not working, then check the logs generated on user creation.
   ...
   To debug:
    ...
    1. Check the [order of logs](/help/communities/using/sync.md#troubleshoot-sling-distribution-in-aem-communities), created on user creation.  
