...
For AEM Communities, it is necessary to configure the Dispatcher to ensure proper functioning of [community sites](/help/communities/using/overview.md#community-sites). Additional configurations are necessary when including features such as Communities enablement and social login.
...
* From the [Web Console](/help/sites/deploying/using/configuring-osgi.md)
...
In the /clientheaders section of `dispatcher.any`, if listing a specific set of headers, it is necessary to include `"CSRF-Token"` in order for the [Enablement feature](/help/communities/using/enablement.md) to work properly.
