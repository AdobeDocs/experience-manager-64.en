...
uuid: 5588a367-cfa4-4481-b852-f84441aed487
contentOwner: Guillaume Carlino
products: SG_EXPERIENCEMANAGER/6.4/SITES
topic-tags: Security
content-type: reference
discoiquuid: db333925-f571-4c91-b4cc-09049485f0fa
...
When the deployment is a [publish farm](../../deploying/using/recommended-deploys.md#tarmkfarm), members need to be able to log in and see their data on any publish node.
...
The user data, along with their [ACLs](../../administering/using/security.md), are stored in the [Oak Core](../../deploying/using/platform.md), the layer below Oak JCR, and are accessed using the [Oak API](/developing/using/reference-materials/javadoc/org/apache/jackrabbit/oak/api/package-tree). With infrequent updates, it is reasonable for user data to be synchronized with other publish instances using [Sling Content Distribution](https://github.com/apache/sling/blob/trunk/contrib/extensions/distribution/README.md) (Sling distribution).
...
* [ACLs](../../administering/using/security.md) set in the user node are included in the sychronization
...
* [AEM Communities updates](/content/help/en/experience-manager/6-4/communities/using/deploy-communities#LatestFeaturePack)
...
* **on author**
    ...
    * access the [Web Console](../../deploying/using/configuring-osgi.md)
...
### 2. Create Authorized User {#create-authorized-user}
...
* **on each publish instance**
    ...
    * sign in with administrator privileges
    * access the [Security Console](../../administering/using/security.md)
...
* [Access Right Management](../../administering/using/user-group-ac-admin.md#accessrightmanagement)
...
### 3. Adobe Granite Distribution - Encrypted Password Transport Secret Provider {#adobe-granite-distribution-encrypted-password-transport-secret-provider}
...
### 5. Adobe Granite Distribution - Diff Observer Factory {#adobe-granite-distribution-diff-observer-factory}
...
When the topology is a [publish farm](../../deploying/using/recommended-deploys.md#tarmkfarm) and user sync has been correctly configured, the *user *and *user profile* is synchronized across the publish farm using Sling distribution.
...
When the [User Administration and Security](../../administering/using/security.md) console is used to add new users in the publish environment, user sync will synchronize the new users and their group membership to other publish instances, if necessary. User sync will also synchronize user groups created through the security console.
...
* on author :
    ...
    * using [CRXDE Lite](../../developing/using/developing-with-crxde-lite.md)

        * look for entries in `/var/sling/distribution/packages`

            * folder nodes named with the pattern `distrpackage_*`

    * using [Package Manager](../../administering/using/package-manager.md)
...
Included in the list is an URL for each publish instance that will run the diagnostics for that instance. The url param `syncUser` is appended to the diagnostics URL with its value set to the *authorized sync user* created in [Step 2](../../administering/using/sync.md#2createauthorizeduser).
...
* on publisher on which users and user groups exist :
    ...
    * [create a package](../../administering/using/package-manager.md#creatinganewpackage) of `/home`
    ...
    * [export the package](../../administering/using/package-manager.md#downloadingpackagestoyourfilesystem)

* on other publish instances :

    * [import the package](../../administering/using/package-manager.md#installingpackages)
