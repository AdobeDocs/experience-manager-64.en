...
uuid: 4a056585-1461-4af8-95ce-b8e80b8780c9
contentOwner: raiman
products: SG_EXPERIENCEMANAGER/6.4/SITES
topic-tags: integration
content-type: reference
discoiquuid: c9098332-10a4-4a97-a650-307d00706f35
index: y
internal: n
snippet: y
---
...
For additional details about logging, see the [Logging](../../deploying/using/configure-logging.md) and [Working with Audit Records and Log Files](../../deploying/using/monitoring-and-maintaining.md#workingwithauditrecordsandlogfiles) pages.
...
* Run Report Importers at a certain time of the day by using CRON expressions for the `ManagedPollingImporter` configurations in the [OSGi console](../../deploying/using/configuring-osgi.md).
...
Analytics has been designed with an inheritance mechanism in mind. Usually, you enable Analytics for a site by adding a reference to an Analytics configuration within the page properties [Cloud Services](../../developing/using/extending-cloud-config.md) tab. The configuration is then inherited to all sub-pages automatically without the need to reference it again unless a page requires a different configuration. Adding a reference to a site also automatically creates several nodes (12 for AEM 6.3 and earlier or 6 for AEM 6.4) of the type `cq;PollConfig` which instantiates PollingImporters used to import Analytics data into AEM. As a result:
...
Firstly, analyzing the ` [error.log](/deploying/using/configure-logging.htm)` might give you some insight about the amount of active or registered PollingImporters. For example:
...
The [DTM](../../administering/using/dtm.md) script tag is not properly included in the page even though the configuration has been referenced in the page properties [Cloud Services](../../developing/using/extending-cloud-config.md) tab.

#### Solution {#solution-2}

To fix the issue, you can try the following:

* Make sure encrypted properties can be decrypted (note that encryption might use a different automatically generated key on each AEM instance). For additional details, also read [Encryption Support for Configuration Properties](../../administering/using/encryption-support-for-configuration-properties.md).
...
For more information about managing ACLs, read the [User Administration and Security](../../administering/using/security.md#permissionsinaem) page.
...
For additional information, read the [Developing for Targeted Content](../../developing/using/target.md#understandingthetargetcomponent) page.
...
* Make sure the customer code loading the DTM-like libraries (which in turn load the Target libraries) is executed synchronously in the [page head](../../developing/using/target.md#enablingtargetingwithadobetargetonyourpages).
...
This issue is most likely an [A4T Analytics Cloud Configuration](../../administering/using/target-configuring.md) provisioning issue.
