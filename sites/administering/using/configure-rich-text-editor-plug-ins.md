...
uuid: 6e663bf3-ba41-4fb0-a582-c33506d92b03
acrolinxdate: 2018-08-16T16 42 39.846-0400
acrolinxlastcheckedby: asgupta
acrolinxpagestatus: yellow
acrolinxreporturl: http //acrolinx.corp.adobe.com 8031/output/en/configure_rich_text_editor_plug_ins_krs_workflow_a9d8be2d910fe9fc_122_report.xml
acrolinxsentences: 322
acrolinxwords: 4117
contentOwner: asgupta
products: SG_EXPERIENCEMANAGER/6.4/SITES
topic-tags: operations
content-type: reference
discoiquuid: cc4bbe4e-d0ad-416c-8249-80c1be8ab8d2
...
For details about the other RTE configurations, see [Configure Rich Text Editor](../../administering/using/rich-text-editor.md).

>[!NOTE]
>
>When working with CRXDE Lite, it is recommended to save the changes regularly using Save All.

## Activate a plug-in and configure the features property {#activate-a-plug-in-and-configure-the-features-property}
...
## Configure the paste modes {#configure-the-paste-modes}
...
1. Navigate to the node `rtePlugins/edit`. See [activate a plug-in](/administering/using/configure-rich-text-editor-plug-ins.html?cq_ck=1534404875505#activateplugin) if the node does not exist.
...
1. In your component, navigate to `<rtePlugins-node>/edit` node. Create the nodes if these do not exist. For more information, see [activate a plug-in](/administering/using/configure-rich-text-editor-plug-ins.html?cq_ck=1534404875505#activateplugin).
...
### Configure the formats allowed when pasting content {#configure-the-formats-allowed-when-pasting-content}
...
1. In your component, navigate to the node `<*rtePlugins-node*>/edit`. Create the nodes if these do not exist. For more details, see [activate a plug-in](/administering/using/configure-rich-text-editor-plug-ins.html?cq_ck=1534360744060#activateplugin).
...
## Configure text styles {#configure-text-styles}
...
>[!NOTE]
>
>Styles can also be defined for [tables or table cells](../../administering/using/configure-rich-text-editor-plug-ins.md#tablestyles). These configurations require separate procedures.

### Enable the Style drop-down selector list {#enable-the-style-drop-down-selector-list}

This is done by enabling the styles plug-in.

1. In your component, navigate to the node `<*rtePlugins-node*>/styles`. Create the nodes if these do not exist. For more details, see [activate a plug-in](/administering/using/configure-rich-text-editor-plug-ins.html?cq_ck=1534360744060#activateplugin).
...
### Specify the style sheet location {#specify-the-style-sheet-location}
...
### Specify the available Styles in the pop-up list {#specify-the-available-styles-in-the-pop-up-list}
...
## Configure the paragraph formats {#configure-the-paragraph-formats}
...
### Enable the Format drop-down selector {#enable-the-format-drop-down-selector}

First enable the paraformat plug-in:

1. In your component, navigate to the node `<*rtePlugins-node*>/paraformat`. Create the nodes if these do not exist. For more details, see [activate a plug-in](/administering/using/configure-rich-text-editor-plug-ins.html?cq_ck=1534360744060#activateplugin).
...
### Specify the available Paragraph Formats {#specify-the-available-paragraph-formats}
...
## Configure special characters {#configure-special-characters}
...
### Define a single character {#define-a-single-character}

1. In your component, navigate to the node `<*rtePlugins-node*>/misctools`. Create the nodes if these do not exist. For more details, see [activate a plug-in](/administering/using/configure-rich-text-editor-plug-ins.html?cq_ck=1534360744060#activateplugin).
...
### Define a range of characters {#define-a-range-of-characters}
...
## Configure table styles {#configure-table-styles}
...
1. Within your component navigate to the node `<*rtePlugins-node*>/table`. Create the nodes if these do not exist. For more details, see [activate a plug-in](/administering/using/configure-rich-text-editor-plug-ins.html?cq_ck=1534360744060#activateplugin).
...
### Configure hidden headers in tables for accessibility {#configure-hidden-headers-in-tables-for-accessibility}
...
## Add dictionaries for the spell checker {#add-dictionaries-for-the-spell-checker}
...
## Configure the history size for undo and redo actions {#configure-the-history-size-for-undo-and-redo-actions}
...
1. Within your component navigate to the node `<*rtePlugins-node*>/undo`. Create these nodes if they do not exist. For more details, see [activate a plug-in](/administering/using/configure-rich-text-editor-plug-ins.html?cq_ck=1534360744060#activateplugin).
...
## Configure the tab size {#configure-the-tab-size}
...
1. In your component, navigate to the node `<*rtePlugins-node*>/keys`. Create the nodes if these do not exist. For more details, see [activate a plug-in](/administering/using/configure-rich-text-editor-plug-ins.html?cq_ck=1534360744060#activateplugin).
...
## Set indent margin {#set-indent-margin}
...
1. Within your component navigate to the node `<*rtePlugins-node*>/lists`. Create these nodes if they do not exist. For more details, see [activate a plug-in](/administering/using/configure-rich-text-editor-plug-ins.html?cq_ck=1534360744060#activateplugin).
...
## Configure the height of editable space {#configure-the-height-of-editable-space}
...
## Configure styles and protocols for links {#configure-styles-and-protocols-for-links}
