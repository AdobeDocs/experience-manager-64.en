...
uuid: 454887c5-5437-4852-992d-08b66951462b
contentOwner: Guillaume Carlino
products: SG_EXPERIENCEMANAGER/6.4/SITES
topic-tags: extending-aem
content-type: reference
discoiquuid: cd7a31ad-9269-45f1-ae9c-7163d5f0092c
...
CQ provides a selection of [standard reports](../../administering/using/reporting.md) most of which are based on a reporting framework.
...
* Is based on the [CQ QueryBuilder](/developing/using/reference-materials/javadoc/com/day/cq/search/QueryBuilder).
...
The developer has to ensure that the query defined for a report returns a node set appropriate for that report. However, the node itself need not hold all the required information, this can also be derived from parent and/or child nodes. For example, the query used for the [User Report](../../administering/using/reporting.md#userreport) selects nodes based on the node type (in this case `rep:user`). However, most columns on this report do not take their data directly from these nodes, but from the child nodes `profile`.
...
* `definitions`  
  ...
  The definitions are specified using an array of nodes (again often named `0`, `1`.. `x`), each having the following properties:
    ...
    * Additional properties are available, dependent on the chart type:
        ...
        * [for the chart type `lineseries`](../../administering/using/reporting.md#displaylimits):
...
This dialog is a standard CQ [dialog](../../developing/using/components.md#dialogs) and can be configured as such (see [CQ.Dialog](/developing/using/reference-materials/widgets-api/index.html?class=CQ.Dialog) for more information).
...
N:cq:editConfig [cq:EditConfig] // [Events and Actions](#eventsandactions)  
N:defaults // [Column Default Values](#columndefaultvalues)  
N:definitions  
N:queryBuilder // [Column Specific Query](#columnspecificquery)  
P:property [String|String[]] // Column Specific Query  
P:subPath // Column Specific Query  
P:secondaryProperty [String|String[]] // Column Specific Query  
P:secondarySubPath // Column Specific Query  
N:data  
P:clientFilter [String] // [Client Filter](#clientfilter)  
P:resolver // [Resolvers and Preprocessing](#resolversandpreprocessing)  
N:resolverConfig // Resolvers and Preprocessing  
N:preprocessing // Resolvers and Preprocessing  
P:type // [Column Specific Definitions](#columnspecificdefinitions)  
  ...
  Provides definitions for the resolver; the options available are dependent on the `resolver` selected:
    ...
    * `pathextension`
        ...
        * `i18n` (optional; type Boolean)  
          Determines whether the resolved value should be *internationalized* (i.e. using [CQ5's internationalization services](../../administering/using/tc-manage.md)).
...
Edit Configuration defines the necessary events for the listeners to detect and the actions to be applied after those events occurs. See the [introduction to component development](../../developing/using/components.md) for background information.
...
Each report type must provide a template. These are standard [CQ Templates](../../developing/using/templates.md) and can be configured as such.
...
1. Drag a component (for this example, **Bundle** in the **OSGi Report** group) from the sidekick to create the first column and [start the report definition](../../administering/using/reporting.md#thebasicsofreportcustomization). [](../../administering/using/reporting.md#thebasicsofreportcustomization)
