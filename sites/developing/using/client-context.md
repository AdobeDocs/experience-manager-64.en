...
uuid: 8ce1221d-e016-4a52-bcd8-fed2b0ebb331
contentOwner: msm-service
products: SG_EXPERIENCEMANAGER/6.4/SITES
topic-tags: personalization
content-type: reference
discoiquuid: 9546fa07-b507-4018-8137-2865da5bd286
...
>[!NOTE]
>
>Client Context has been superseded by ContextHub in the touch UI. Please see the [related documentation](../../developing/using/contexthub.md) for details.
...
* A [javascript API](../../developing/using/ccjsapi.md) for interacting with session stores.
...
For information about opening Client Context, configuring the information that it displays, and simulating the user experience, see [Client Context](../../administering/using/client-context.md).
...
* The server (see [JSONP Store](../../administering/using/client-context.md#main-pars-variable-8) for storing information from 3rd-party sources)

The Client Context framework provides a [javascript API](../../developing/using/ccjsapi.md) that you can use to interact with session stores to read and write user data, and listen and react to store events. You can also create session stores for user data that you use for content targeting or other purposes.
...
For a description of the installed Context Store Components that you can add to Context Store, see [Available Client Context Components](../../administering/using/client-context.md#main-pars-title).
...
Profile information is also used for delivering [targeted content](../../authoring/using/content-targeting-touch.md).
...
* ` [CQ_Analytics.SessionStore](../../developing/using/ccjsapi.md#cqanalyticssessionstore)`: These objects reside only in the page DOM. The data is created and persisted during the lifetime of the page.
* ` [CQ_Analytics.PerstistedSessionStore](../../developing/using/ccjsapi.md#main-pars-title-1)`: These objects reside in the page DOM and are persisted either in browser storage or cookies. The data is available across pages and across user sessions.

The API also provides extensions of these classes that are specialized for storing JSON data or JSONP data:

* Session-only objects: [CQ_Analytics.JSONStore](../../developing/using/ccjsapi.md#main-pars-title-30) and [CQ_Analytics.JSONPStore](../../developing/using/ccjsapi.md#main-pars-title-54).

* Persisted objects: [CQ_Analytics.PersistedJSONStore](../../developing/using/ccjsapi.md#main-pars-title-3) and [CQ_Analytics.PersistedJSONPStore](../../developing/using/ccjsapi.md#main-pars-title-20).

#### Creating the Session Store Object {#creating-the-session-store-object}

The javascript of your client library folder creates and initializes the session store. The session store must then be registered using Context Store Manager. The following example creates and registers a [CQ_Analytics.SessionStore](../../developing/using/ccjsapi.md#cqanalyticssessionstore) object.
...
For storing JSON data, the following example creates and registers a [CQ_Analytics.JSONStore](../../developing/using/ccjsapi.md#cqanalyticssessionstore) object.
...
Obtain a session store object to read or write data to the store. [CQ_Analytics.ClientContextMgr](../../developing/using/ccjsapi.md#main-pars-title-44) provides access to stores based on the store name. Once obtained, use the methods of the [CQ_Analytics.SessionStore](../../developing/using/ccjsapi.md#main-pars-title) or [CQ_Analytics.PersistedSessionStore](../../developing/using/ccjsapi.md#main-pars-title-1) to interact with store data.
...
The session stores are built on the `Observable` pattern. They extend [ `CQ_Analytics.Observable`](../../developing/using/ccjsapi.md#main-pars-title-62) that provides the ` [addListener](../../developing/using/ccjsapi.md#main-pars-title-67)` method.
...
Use the [CQ_Analytics.ClientContextUtils](../../developing/using/ccjsapi.md#main-pars-title-69) object's [onStoreRegistered](../../developing/using/ccjsapi.md#main-pars-title-64) and [onStoreInitialized](../../developing/using/ccjsapi.md#main-pars-title-63) methods to access session stores only when they are available. These methods enable you to register event listeners that react to session registration and initialization events.
...
See ` [CQ_Analytics.PersistedSessionStore.setNonPersisted(propertyName)](../../developing/using/ccjsapi.md#main-pars-title-2)`
