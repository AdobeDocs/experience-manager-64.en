...
uuid: a6263e00-eac3-4da5-ba61-b8a162d7925a
contentOwner: User
products: SG_EXPERIENCEMANAGER/6.4/SITES
topic-tags: extending-aem
content-type: reference
discoiquuid: 4fce1f8e-3271-49b6-9a9c-1bef761aaa66
...
>[!NOTE]
>
>This page should be read in conjunction with [Reusing Content: Multi Site Manager](../../administering/using/msm.md).
...
* [com.day.cq.wcm.msm.api](/developing/using/reference-materials/javadoc/com/day/cq/wcm/msm/api/package-frame)
* [com.day.cq.wcm.msm.commons](/developing/using/reference-materials/javadoc/com/day/cq/wcm/msm/commons/package-frame)

The main MSM API objects interact as follows (see also [Terms Used](../../administering/using/msm.md#termsused)):

![](assets/chlimage_1-45.png)

* **`Blueprint`** 
  A `Blueprint` (as in [blueprint configuration](../../administering/using/msm.md#sourceblueprintsandblueprintconfigurations)) specifies the pages from which a live copy can inherit content. 
...
Create custom synchronization actions to use with your rollout configurations. Create a synchronization action when the [installed actions](../../administering/using/msm-sync.md#installedsynchronizationactions) do not meet your specific application requirements. To do so, create two classes:

* An implementation of the [ `com.day.cq.wcm.msm.api.LiveAction`](/developing/using/reference-materials/javadoc/com/day/cq/wcm/msm/api/LiveAction) interface that performs the action. 
* An OSGI component that implements the [ `com.day.cq.wcm.msm.api.LiveActionFactory`](/developing/using/reference-materials/javadoc/com/day/cq/wcm/msm/api/LiveActionFactory) interface and creates instances of your `LiveAction` class.
...
<p>Create custom synchronization actions to use with your rollout configurations. Create a synchronization action when the <a href="../../administering/using/msm-sync.md#installedsynchronizationactions">installed actions</a> do not meet your specific application requirements. To do so, create two classes:</p>
<ul>
<li>An implementation of the <a href="/developing/using/reference-materials/javadoc/com/day/cq/wcm/msm/api/LiveAction"><span class="code">com.day.cq.wcm.msm.api.LiveAction</span></a> interface that performs the action. </li>
<li>An OSGI component that implements the <a href="/developing/using/reference-materials/javadoc/com/day/cq/wcm/msm/api/LiveActionFactory"><span class="code">com.day.cq.wcm.msm.api.LiveActionFactory</span></a> interface and creates instances of your <span class="code">LiveAction</span> class.</li>
</ul>
...
The parameter of the ` [LiveActionFactory](/developing/using/reference-materials/javadoc/com/day/cq/wcm/msm/api/LiveActionFactory).createAction` method is a `Resource` object. This `Resource` object represents the `cq:LiveSyncAction` node for this live action in the rollout configuration; see [Creating a Rollout Configuration](../../administering/using/msm-sync.md#creatingarolloutconfiguration). As usual when using a configuration node, you should adapt it to a `ValueMap` object:
...
* A [ `Resource`](/developing/using/reference-materials/javadoc/org/apache/sling/api/resource/Resource) object that represents the source of the Live Copy.
* A `Resource` object that represents the target of the Live Copy.
* The [ `LiveRelationship`](/developing/using/reference-materials/javadoc/com/day/cq/wcm/msm/api/LiveRelationship) object for the live copy.
...
>[!NOTE]
>
>The `Resource` arguments may be `null` or `Resources` objects that do not adapt to `Node` objects, such as [ `NonExistingResource`](/developing/using/reference-materials/javadoc/org/apache/sling/api/resource/NonExistingResource) objects.
...
>[!NOTE]
>
>See also the [best practices for customizing rollouts](../../administering/using/msm-best-practices.md#customizingrollouts).
...
1. Click **Edit**.
1. In the **Rollout Config** dialog, select the ** [Sync Trigger](../../administering/using/msm-sync.md#rollouttriggers)** to define the action that causes the rollout to occur.
...
1. Click **Create** then **Create Node**. Then configure the following node properties and click **OK**:
    ...
    * **Name**: The node name of the synchronization action. The name must be the same as the **Action Name** in the table under [Synchronization Actions](../../administering/using/msm-sync.md#installedsynchronizationactions), for example `contentCopy` or `workflow`.
    
    * **Type**: `cq:LiveSyncAction`

1. Select the action node just created and add the following property to the node:

    * **Name**: The property name of the action. The name must be the same as the **Property Name** in the table under [Synchronization Actions](../../administering/using/msm-sync.md#installedsynchronizationactions), for example `enabled`.  
    
    * **Type**: String  
    
    * **Value**: the property value of the action. For valid values, see the **Properties** column in [Synchronization Actions](../../administering/using/msm-sync.md#installedsynchronizationactions), for example `true`.
...
* For information about the adobe-public profile, see [Obtaining the Content Package Maven Plugin](../../developing/using/vlt-mavenplugin.md#obtainingthecontentpackagemavenplugin)
...
1. Start Eclipse and [import the Maven project](../../developing/using/howto-projects-eclipse.md#importthemavenprojectintoeclipse).
...
1. Create and configuration a [Rollout Configuration with the standard procedure](../../administering/using/msm-sync.md#creatingarolloutconfiguration) - and using the properties:
...
[Create a live copy](../../administering/using/msm-livecopy.md#creatingalivecopyofapage) of the English/Products branch of the We.Retail Reference Site using your rollout configuration:
...
* Contact Email:
    ...
    * Is excluded from the rolled out properties; see [Excluding Properties and Node Types from Synchronization](../../administering/using/msm-sync.md#main-pars-title-490730743).
