...
uuid: 90eaa70f-131b-4106-8aeb-0a962cd926ba
contentOwner: User
products: SG_EXPERIENCEMANAGER/6.4/SITES
content-type: reference
topic-tags: deploying
discoiquuid: 4ca6d5b2-2616-449d-b10c-36232ab2767d
...
>
>For info on more available deployments, see the [Recommended Deployments](../../deploying/using/recommended-deploys.md) page.
...
1. Go to the installation folder of the primary instance and:
    ...
    1. Create a file called `org.apache.jackrabbit.oak.segment.standby.store.StandbyStoreService.config` in the same location and configure it accordingly. For more information on the configuration options, see [Configuration](../../deploying/using/tarmk-cold-standby.md#main-pars-title-9).
...
1. Create a new Apache Sling Logging Logger for the **org.apache.jackrabbit.oak.segment** package. Set log level to “Debug” and point its log output to a separate logfile, like */logs/tarmk-coldstandby.log*. For more information, see [Logging](../../deploying/using/configure-logging.md).
...
1. Create and start a new standby instance. For more info, see the procedure above on [Creating an AEM TarMK Cold Standby Setup](../../deploying/using/tarmk-cold-standby.md#main-pars-title-2).
...
1. Install the hotfix on the primary instance. For more details on how to install a hotfix, see [How to Work With Packages](../../administering/using/package-manager.md).
...
1. Reconfigure the newly created clone to act as a cold standby instance. For additional details, see [Creating an AEM TarMK Cold Standby Setup.](../../deploying/using/tarmk-cold-standby.md#creatinganaemtarmkcoldstandbysetup)
1. Start both the primary and the cold standby instances.

## Monitoring {#monitoring}

The feature exposes information using JMX or MBeans. Doing so you can inspect the current state of the standby and the master using the [JMX console](../../administering/using/jmx-console.md). The information can be found in an MBean of `type org.apache.jackrabbit.oak:type="Standby"`named `Status`.
...
>[!NOTE]
>
>If you run [Online Revision Cleanup](../../deploying/using/revision-cleanup.md) on the primary instance, the manual procedure presented below is not needed. Addittionally, if you are using Online Revision Cleanup, the `cleanup ()` operation on the standby instance will pe performed automatically.
...
1. Stop the standby process on the standby instance by going to the JMX Console and using the **org.apache.jackrabbit.oak: Status ("Standby")** bean. For more info on how to do this, see the above section on [Monitoring](../../deploying/using/tarmk-cold-standby.md#monitoring).  

1. Stop the primary AEM instance.
1. Run the oak compaction tool on the primary instance. For more details, see [Maintaining the Repository](../../deploying/using/storage-elements-in-aem-6.md#maintaining-the-repository).
...
1. Run cold standby repository maintenance as described in the section [above](../../deploying/using/tarmk-cold-standby.md#coldstandbyrepositorymaintenance).
1. After the maintenance process has completed and the instances have been restarted:

    * On the primary, run the data store garbage collection via the relevant JMX bean as described in [this article](../../administering/using/data-store-garbage-collection.md#runningdatastoregarbagecollectionviathejmxconsole).
