...
uuid: 6d2c67c2-f960-4c31-abc6-a5a5ad062121
...
discoiquuid: 01623300-d6a6-4b83-9f2d-b7aa724973ea
...
sha1: 63e2399507bc954c06ef164519aaadca98eefc82
...
>
>For info on more available deployments, see the [Recommended Deployments](recommended-deploys.md) page.
...
>[!CAUTION]
>
>There is a maximum limit to the binary size that can be synced between the primary and standby instances. The limit is **2.14 GB** and no binaries bigger than this value can be synced between the instances. A workaround to this limit would be to use a shared data store. For more details, see [Data Store Configurations](data-store-config.md).
>
...
![](assets/tarmk-cold-standby/chlimage_1.png) 
...
>[!NOTE]
>
>If you run [Online Revision Cleanup](revision-cleanup.md) on the primary instance, the manual procedure presented below is not needed. Addittionally, if you are using Online Revision Cleanup, the `cleanup ()` operation on the standby instance will pe performed automatically.
...
1. Run the `cleanup()` operation on the standby instance, using the same JMX bean as described in the first step.
   ...
   >[!NOTE]
   >
   >Restarting the standby instance before running `cleanup()` might help with the efficiency of the operation.
