...
uuid: aa2d21f6-1339-4f02-9306-350d7b667312
content-encoding: ISO-8859-1
acrolinxstatus: not_checked
aemsrcnodepath: /content/help/en/experience-manager/6-4/sites/deploying/using/repository-restructuring-in-aem64
contentOwner: sarchiz
cq-designpath: /etc/designs/help
cq-lastmodified: 2018-07-27T04 41 49.266-0400
cq-lastmodifiedby: sarchiz
cq-lastreplicated: 2018-07-27T04 41 49.350-0400
...
discoiquuid: 2314d0c6-54e9-472c-9acf-1ac1ecce25e8
donotlocalize: false
firstPublishExternalDate: 2018-04-03T10:34:00.981-0400
herogradient: light
jcr-created: 2018-04-03T07 28 59.421-0400
jcr-createdby: admin
jcr-ischeckedout: true
jcr-language: en_us
lastPublishExternalDate: 2018-07-27T04:41:49.242-0400
lochandoffdate: 2018-04-30T03 29 06.878-0400
...
publishexternaldate: 2018-07-27T04 41 49.242-0400
publishExternalURL: https://helpx.adobe.com/experience-manager/6-4/sites/deploying/using/repository-restructuring-in-aem64.html
redirecttarget: /content/help/en/experience-manager/6-4/sites/deploying/using/repository-restructuring.html
sha1: 1a44aa853ebebdcfeb48ee3d3b119e5bed3bc885
sidecolumn: left
topicBrowsingSortDate: 2018-07-27T04:41:49.242-0400
...
To that end, beginning in AEM 6.4 and to be continued in future releases, content is being restructured out of `/etc` to other folders in the repository, along with guidelines on what content goes  adhering to the following high-level rules:

* AEM product code will always be placed in `/libs`, which must not be overwritten by custom code
* code should be placed in `/apps`, `/content`, and `/conf`
...
In most cases,  compatibility to the old locations is maintained after upgrading to AEM 6.4. This is achieved by the old locations being preserved and respected by product code, in addition to the new locations being added. In most cases, conditional logic is used to check if the legacy folder exists and to read content from there instead of the new location.

On their own timeline after the 6.4 upgrade, customers are encouraged to remove the old locations so content in the new locations is used. The tables below  instructions to adhere to the new content structure per-location.

>[!NOTE]
>
><p>An in-place upgraded instance will include old content locations in addition to the new locations. A fresh developer install will only include the new locations.</p>
...
In general, but with a few exceptions, content under this section can be extended using Apache Sling's [  Configuration](https://sling.apache.org/documentation/bundles/context-aware-configuration/context-aware-configuration.html) feature.

In a nutshell,  Configuration allows content in one part of the repository to be overlayed multiple times by content in other parts of the repository. For example, content in `/libs` can be overlayed by  in `/apps`, which can then be overlayed by  in `/conf`. Moreover, content in a global folder under `/conf` can be overlayed by a specific "tenant" or "site" (e.g. `/conf/we-retail/settings`).

Typically,  Configuration is used as a strategy for extending feature configurations, but there are cases where it's used by other content types.

The table below includes an additional column named "Configuration Type" to explain the extent to which a  can be overlayed. Here is additional detail on these configuration types:
...
<table border="1" cellpadding="1" cellspacing="0" width="100%"> 
 <tbody> 
  ...
  <tr> 
   ...
   <td><p>Lazy Content Migration Task is available in the form of <span class="code">CQ64CommunitiesConfigsCleanupTask</span>.</p> <p>For more info, see the <a href="lazy-content-migration.md" target="_blank">Lazy Migration documentation</a>.</p> </td> 
  </tr> 
  <tr> 
   <td>AEM Communities</td> 
